...
| [`e05_ising_criticality.ipynb`](e05_ising_criticality.ipynb) | Ising rings and a 3×3 lattice, exact E_π[φ]: micro-level 2023 saturates in the ordered phase (no critical peak — contra the proxy literature), but coarse-graining *restores* criticality — macro φ peaks near the transition under both formalisms | done |
...
| [`e07_sensitivity_vs_lesions.ipynb`](e07_sensitivity_vs_lesions.ipynb) | Three probes of where integration lives: gradients are exact but cell-local, edge lesions saturate on needles, unit knockouts annihilate φ everywhere — no probe validates another | done |
| [`e08_unfolding_floor.ipynb`](e08_unfolding_floor.ipynb) | The unfolding floor is real and behavior-specific (0 to 2.08 ibits across behaviors; control certifies the optimizer finds zero when zero exists) — φ_s = behavioral requirement + free surplus | done |
| [`e09_adversarial_axiomatics.ipynb`](e09_adversarial_axiomatics.ipynb) | The knobs × desiderata battery, whole-universe: effect surprisal kills determinism, delta repertoires (not normalization) create the frozen class, occupancy fixes isolation only — and the cause-only cap passes everything | done |
| [`e10_closing_the_theorems.ipynb`](e10_closing_the_theorems.ipynb) | C(n) attained through n = 6 (99.8% from 8 seeds); the weighted-max conjecture dies to the n = 3 coincidence factory; the n = 2 optimum is a MIP-surprisal triple point, now derivable | done |
| [`e11_in_degree_ladder.ipynb`](e11_in_degree_ladder.ipynb) | The attainment iff closed exactly (double minterm saturation, XOR = 0 over the universe); the repaired measure's stochastic ceiling is 1.39+ (MIP-surprisal crossing); the n = 4 ladder search failed its own anchors and is requeued | done |
| [`e12_quantum_phi.ipynb`](e12_quantum_phi.ipynb) | Empirical legs of the QIIA paper: the oracle lemma exact at machine precision, statevector Dürr–Høyer recovering exact φ_s with ~K^0.56 queries, and the cut census tracking the Fubini numbers | done |
| [`e13_separable_dp.ipynb`](e13_separable_dp.ipynb) | The cut objective is exactly separable (10⁻¹⁶ vs internals) and Dinkelbach-reducible, but the free per-unit relaxation is loose on 100% of systems — the collapse must respect the (now exactly characterized) partition-with-modes family | done |
| [`e14_tractable_islands.ipynb`](e14_tractable_islands.ipynb) | Tractable islands: sparse cut-dedup exact (with the phantom-connection quirk), ring ceiling 3.75 ≪ 12 — and rule 110, the universal computer, has φ_s = 0 at 31/32 states: computation and integration decouple | done |
